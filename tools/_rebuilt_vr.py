...
def _build_pipe_groups(group_names_csv: str | None = None) -> dict[str, list[str]]:
    ...
    Build deterministic grouped-loss mapping.
            ...
            if allow_heat_demand_slack and max_heat_demand_slack_frac > 0.0 and Q_demand is not None:
                def _demand_slack_bounds(m, t, _Q=Q_demand, _frac=max_heat_demand_slack_frac):
                    q_t = 0.0
                    try:
                        q_t = max(0.0, float(pyo.value(_Q[t])))
                    except Exception:
                        q_t = 0.0
                    return (0.0, _frac * q_t)
                ...
                logger.info(
                    ...
                    f"(max {max_heat_demand_slack_frac:.1%}, penalty={demand_slack_penalty:.0e} €/MWh)"
                )
...
def aggregate_source_measurements(hist: pd.DataFrame) -> pd.DataFrame:
    ...
    Uses:
      ...
      - V_X_flow_rate: volume flow per consumer [mÂ³/h]
      - V_X_return_temp: return temperature per consumer
      - outdoor_temp_C: ambient temperature
      - WRG_1 Â°C: heat recovery source temperature
    """
...
# ---------------------------------------------------------------------------
    ...
    parser.add_argument("--grouped-loss-grid", type=str, default="0.6,0.8,1.0,1.2,1.4,1.7")
    parser.add_argument("--grouped-return-loss-grid", type=str, default="0.6,0.8,1.0,1.25,1.5,1.8")
    parser.add_argument("--calib-max-candidates-total", type=int, default=16)
...
# MIQP/NLP model run (milp_linearize=false for temperature propagation KPIs)
    ...
    profile_overrides = _validation_profile_overrides(validation_profile)
...
def _build_ground_temp_dict_for_horizon(
    ...
    where D = sqrt(2α/ω) (damping depth), α = soil thermal diffusivity.
...
# ---------------------------------------------------------------------------
    ...
    if run_s1 and not args.dry_run:
        ...
        if data_path.exists():
            ...
            calibration_meta["return_clusters"] = cluster_meta
            print(f"  Representative weeks: {list(weeks.keys())}")
    ...
    if run_s1 and not args.miqp_only:
        ...
        if measured_agg is not None and sim_legacy is not None:
            kpis = compute_stage1_kpis(measured_agg, sim_legacy, bc_info)
            if legacy_used_stale_dispatch:
                kpis["stage1_used_stale_dispatch"] = True
            ...
            if not args.no_calibrate:
                if args.no_miqp:
                    ...
                    print(
                        "  [CALIBRATE] Skipping MILP per-pipe U drift in MIQP mode "
                        "(identifiability guard active)."
                    )

            # Plots (MILP)
            print("  Generating Stage 1 plots (MILP)...")
            plot_stage1_pack(
                pack=plot_pack,
                measured=measured_agg,
                simulated=sim_legacy,
                bc_info=bc_info,
                weeks=weeks,
                out_dir=OUT_DIR,
                kpis_for_hist=kpis,
    ...
    # Ã¢â€â‚¬Ã¢â€â‚¬ Step 3: Stage 1 MIQP Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
    if run_s1 and not args.no_miqp:
        print("\n[3/6] Stage 1 Ã¢â‚¬â€ MIQP Network validation (temperature propagation)")
        ...
        if not args.skip_model:
            ...
            miqp_success, miqp_run_reason = run_miqp_model(
                dry_run=args.dry_run,
                bc_info=bc_info,
                calibrated_u=calibrated_u if calibrated_u else None,
                node_return_tuning=primary_node_tuning,
                fix_binaries_from_milp=primary_fix_from_milp,
                fix_binaries_from_warmstart=primary_fix_from_warmstart,
                relaxed_first=bool(args.miqp_relaxed_first),
                allow_relaxed_binary_reuse=True,
                time_limit_s=args.miqp_time_limit,
                mip_gap=primary_gap,
# [MISSING 4940]
# [MISSING 4941]
# [MISSING 4942]
            ...
            if pwl_check.get("ok"):
                print(f"  [PWL-CHECK] OK ({pwl_check.get('checked_pipes', 0)} pipes)")
                        ...
                        if used >= max_total:
                            ...
                            candidate_trace.append({"tag": tag, "status": "budget_exhausted"})
                            return None
                        budget["used_candidates"] = used + 1
    if run_miqp and level_norm in ("L3NLP", "ALL") and effective_return_model_mode != "stateful_v2":
        print("  [L3NLP] return model forced to stateful_v2")
        effective_return_model_mode = "stateful_v2"
