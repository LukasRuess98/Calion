...
# ---- alternative clusterings (tree-valid by construction) ----
ALTS = {
    "coarse4": {1: "1", 2: "1", 3: "1", 4: "2", 5: "2", 6: "2", 7: "2", 8: "2",
                9: "3", 10: "3", 11: "3", 12: "4", 13: "4", 14: "4", 15: "4"},
    "fine10":  {1: "A", 2: "B", 3: "C", 4: "D", 5: "D", 6: "E", 7: "E", 8: "E",
                9: "F", 10: "G", 11: "G", 12: "H", 13: "I", 14: "J", 15: "J"},
    "shift7":  {1: "A", 2: "A", 3: "B", 4: "C", 5: "C", 6: "C", 7: "D", 8: "D",
                9: "E", 10: "E", 11: "F", 12: "F", 13: "F", 14: "G", 15: "G"},
}


def _random_contiguous(l3, k, rng):
    """Cut k-1 random tree edges -> k connected components -> valid partition."""
    parent, pipes = _tree(l3)
    edges = list(pipes.keys())
    cut = set(rng.sample(edges, k - 1))
...
def emit(n_null):
    ...
    for name, part in ALTS.items():
        ...
        if not diag["valid_tree"]:
            print(f"[SKIP] alt {name} not tree-valid: {diag['parent_zone']}"); continue
        cfg["scenario"]["name"] = f"Memmingen L2 alt {name}"
        (OUTDIR / f"L2_alt_{name}.yaml").write_text(
            yaml.dump(cfg, allow_unicode=True, sort_keys=False), encoding="utf-8")
        written.append(f"L2_alt_{name} ({diag['n_zones']} zones)")
    # null: random contiguous 7-zone partitions
    rng = random.Random(1234)
    made = 0
    for i in range(n_null):
        part = _random_contiguous(l3, 7, rng)
        cfg, diag = aggregate(part, l3, l2)
        if not diag["valid_tree"]:
            continue
        cfg["scenario"]["name"] = f"Memmingen L2 null {i:02d}"
        (OUTDIR / f"L2_null_{i:02d}.yaml").write_text(
            yaml.dump(cfg, allow_unicode=True, sort_keys=False), encoding="utf-8")
        made += 1
    print(f"[EMIT] wrote {len(written)} alt + {made} null configs to {OUTDIR}/")
    for w in written:
        print("   +", w)
