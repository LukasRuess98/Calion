...
untested. This study provides a controlled comparison: across
184 optimization instances on a validated industrial network and
...
dispatch with given capacities), practical implication could been determined: resolve the network graph before refining the physics.
...
District heating networks (DHN) for the industrial usage are central to European decarbonisation
strategies, integrating heat pumps (HP), combined heat and power (CHP), and thermal storage (TES) within
multi-source systems~\citep{Lund2014_4GDH,Werner2017}.
Operational optimization of such networks increasingly relies on
mathematical programming. The level of detail in the modelling of heating networks depends heavily on whether the topology and thermohydraulics are simplified:
...
resolve the physical pipe-by-pipe network, capturing thermal losses
 5--15\,\% of annual heat
supply~\citep{Frederiksen2013,Talebi2016}, pressure-dependent
...
A common assumption in the community is that higher thermo-hydraulic accuracy and finer spatial resolution both yield proportional accuracy
gains justifying the associated computational cost. Yet no
controlled quantitative evidence exists to confirm or refute this
proportionality, nor to identify which modelling dimension delivers
the dominant return on complexity.
...
\begin{enumerate}[nosep]
  ...
  \item A quantitative guide to selecting piping topology and
        thermohydraulic design based on observable network characteristics.
\end{enumerate}
...
\begin{description}[style=nextline, nosep, leftmargin=1.5em]
  \item[Contribution 1 / RQ1 Topology effect:]
    A five-level taxonomy (L1, L2, L3, $\Lplus$, $\Lnl$) and controlled topology comparison within a unified MILP
    framework where L2 and L3 share identical thermo-hydraulics. This
    isolates topology as the sole experimental variable and is
    validated against measured industrial operational data.
    \emph{How much does topology abstraction alone change annual
    operational cost and \ce{CO2} emissions?}

  \item[Contribution 2 / RQ2 thermo-hydraulic effect:]
    A controlled thermo-hydraulic accuracy comparison (L3 vs.\ $\Lplus$)
    ...
    have a significant impact on load distribution once the topology has already been determined?}
  ...
  \item[Contribution 4 / RQ4 Generalisability:]
    ...
    topology and/or extended physics representation is warranted?}
\end{description}
...
In the field of dispatch strategies for DHN numerous studies have been published. Given the wide range of applications, MILP modelling strategies for the topologies of DHN are a frequent topic of discussion (\Cref{sec:rw_milp_topo}). The linearisation of thermo-hydraulic networks varies as well across these studies (\Cref{sec:rw_thermohydraulic}).

\subsection{MILP optimization and spatial scale in DH systems}
\label{sec:rw_milp_topo}

MILP has become the dominant methodology for operational
optimization of DH systems~\citep{Fang2016,Guelpa2019review}.
Open-source frameworks like oemof~\citep{Hilpert2018_oemof},
PyPSA~\citep{Brown2018_pypsa}, FINE~\citep{Welder2018_FINE},
Calliope~\citep{Pfenninger2018_calliope},
urbs~\citep{Dorfner2016_urbs} support multi-carrier energy
system optimization but share common limitations for detailed DH
...
spatial aggregation. Single-node models are standard in
national-scale analyses~\citep{Connolly2014_EnergyPLAN,
...
geographic clusters~\citep{Wirtz2020,Mertz2016,Buffa2019};
...
data~\citep{Vesterlund2017,Schweiger2017,vanderHeijde2017,
...
results~\citep{Wirtz2020,Buffa2019}, but no scalar metric links
...
properties~\citep{Frederiksen2013} and are MILP-compatible when
temperatures are fixed parameters. The Darcy--Weisbach equation,
...
linearisation~\citep{Bordin2016,Mertz2016} or second-order cone
relaxations~\citep{Blommaert2020}. \citet{Hari2024} provide a
comprehensive thermal network flow optimization framework
integrating Darcy--Weisbach pressure drop with explicit pump
placement. Reported pumping costs range from 1--5\,\% of thermal
energy costs~\citep{Frederiksen2013}.

Temperature propagation along pipes introduces exponential and
bilinear terms~\citep{vanderHeijde2017}; \citet{Maldonado2024}
...
long pipes ~\citep{vanderHeijde2017,Giraud2015}; the
...
NLP~\citep{Ommen2016}: quadratic constraints are handled natively
by modern solvers while integer variables retain unit-commitment
logic. However, MIQCP problems lack LP-relaxation tightness,
potentially increasing solve times. The magnitude of the
linearisation error in a DH dispatch context i.e., the cost gap
between Piece wise linear (PWL)-MILP and native MIQCP formulations has not been published.
...
reference in a controlled DH dispatch setting. The present paper is the first controlled comparison that addresses
all three dimensions within a single framework.
...
\begin{table*}[!tbp]
\centering
\caption{DH optimization studies with
...
each level. The five variants are built up as follows inspired by \cite{Blommaert2020}.

\begin{table}[htbp]
\centering
\caption{Model scope across all five variants. ``PWL'': piecewise-linear;
``Q/B'': native quadratic/bilinear constraint.}
...
\quad Hourly grid $e$-factors
  & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark \\
\midrule
\multicolumn{6}{@{}l@{}}{\textit{Excluded (all levels)}} \\[1pt]
...
resulting pumping power. Transport delay is configured but bypassed:
...
we additionally define L1$_{\mathrm{topo}}$: full network routing
...
across nodes. We quantify this via (i)~the \textbf{Gini
coefficient} of nodal annual demand shares and (ii)~the
\textbf{top-3 demand share}---both widely interpretable and with
sufficient dynamic range on realistic networks. For the primary
...
[MW] are grid import and export at time~$t$;
...
$\lambda_g^{\mathrm{fuel}}$ [EUR/MWh] its specific fuel cost;
...
demand charge $\lambda^{\mathrm{dem}}$.
...
[MW] are discharge and charge of storage unit~$s$, and
...
optimised, the losses are \emph{fixed parameters}---a key
...
bilinear terms in the optimization model.
...
\citet{Kus2025} and \citet{Maldonado2024}, we adopt a two-stage
strategy.
...
gates defined \emph{ex ante} from published practice.
...
All five variants are implemented in Python~3.10 using
\citet{pyomo}, instantiated from a single parameterised codebase
via YAML configuration files. Solver: Gurobi~10.0 with MIP gap
${\leq}\,0.5\,\%$ for both MILP and MIQCP
(\texttt{NonConvex=2}); identical random seeds. $\Lnl$ is
warm-started from the $\Lplus$ solution.

The $\Lplus$ formulation adds ${\approx}\,614{,}000$ continuous
variables; $\Lnl$ adds ${\approx}\,245{,}000$ quadratic
constraints. A single 744\,h MIQCP solve requires ${>}\,4$\,h at
1.99\,\% gap; the full-year formulation is intractable on
single-workstation hardware. The synthetic parametric study is therefore
restricted to L1/L2/L3/$\Lplus$, while the linearisation error is
...
The case study comprises data from a real industrial heating network and several synthetic networks to generate generalizable results.
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{paper1_dh_fidelity/F2_network_topology.png.svg}
...
plus 4~primary-case levels, totalling 184~optimization runs.
...
Of the 14~network nodes, 6 enter the gate evaluation
(``validation nodes'', cf.\ \Cref{tab:val_targets}); the remaining
8 are excluded for two distinct reasons (per-node MAE breakdown in
...
\begin{figure*}[!t]
  \centering
  \includegraphics[width=\textwidth]{paper1_dh_fidelity/FV1_validation_timeseries.png}
...
$[2.5,\,3.0]$; electrode-boiler price-response correlation
$r = 0.47$; per-step energy closure below 0.3\,\%; TES SOC in
$[0.18,\,0.94] \cdot \bar{E}_s$. Solver consistency: $\Lnl$ with
quadratic constraints deactivated reproduces L3 within
$0.004\,\%$.
...
hourly predictions fall within the ${\pm}$\SI{1.5}{\celsius}
per-step gate. \Cref{fig:val_scatter} presents the scatter plot of
simulated versus measured supply temperature;
...
accounts for 10.5$\%$ (63\,t\,\ce{CO2}/yr), while
L2$\to$L3 contributes the remaining 2.5~$\%$ (11\,t\,\ce{CO2}/yr).
...
operating conditions negligible for planning-accuracy dispatch. \Cref{fig:cost_waterfall} visualises the resulting cost decomposition across all five model levels on the primary case. L3 and 
$\Lplus$ yield the highest total cost at approximately 170 k€/yr, whereas L1 produces the lowest at 147 k€/yr.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{paper1_dh_fidelity/F5_cost_waterfall.png}
  \caption{Cost difference: topology resolution
  (L1$\to$L2$\to$L3) dominates; the extended-physics
  (L3$\to\Lplus$) and linearisation effects ($\Lplus\to\Lnl$)
  are marginal. Values on the primary case.}
...
pressure drop reveals an unambiguous driver
(\Cref{fig:synth_sweep}b): the heat-loss component accounts for
nearly the entire gap (median $+20.1\,\%$), while routing and
pressure drop remain at or below the MIP-gap noise floor. The $+0.02\,\%$ pressure-drop component here measures its share
\emph{within} the L1$\to$L3 gap (denominator: L3 total cost);
the $+0.11\,\%$ reported in \Cref{sec:results_extended} measures
the isolated L3$\to\Lplus$ increment (same denominator, different
comparison pair). Both confirm that hydraulic physics are marginal. The
surprising finding is not that losses matter this is physically
obvious but that \emph{everything else does not}: routing
constraints, pressure-drop physics, and nonlinear formulations each
contribute less than 1\,\% even at 15\,km pipe length.

\paragraph{Sensitivity to parametric study variables.}
Pipe length is the dominant predictor of the L1$\to$L3 gap. Within
pipe-length bands, node count, demand HI, and storage ratio show no
clear monotone trends. \Cref{fig:synth_surface} confirms this
dominance across all 36~configurations: at 1\,km the gap remains
below 5\,\%, rising to 14--20\,\% at 5\,km and exceeding 35\,\% at
15\,km. Storage capacity has a secondary effect---larger reservoirs
allow more temporal dispatch flexibility that the copperplate model
cannot capture. The nearly vertical gradient confirms that network
...
reversal for L2--L3 ($-2.2\,\%$). At reduced thermal demand,
pipe losses shrink as a fraction of load; simultaneously, the
directional bias of L2's zonal averaging (which smooths local
peaks) flips sign because the HP at j$_{12}$ now serves a larger
share of reduced total demand, reducing average transport
distance. This is the only scenario in which L2 \emph{over}-
estimates cost relative to L3 suggesting that L2 may be
...
Open-source frameworks for multi-energy system optimization
including oemof~\citep{Hilpert2018_oemof},
PyPSA~\citep{Brown2018_pypsa}, FINE~\citep{Welder2018_FINE},
Calliope~\citep{Pfenninger2018_calliope}, and
urbs~\citep{Dorfner2016_urbs} typically represent thermal
...
\begin{enumerate}[nosep]
  \item \textbf{L2 as a low-cost minimum.} Adding per-segment
        steady-state losses (\Cref{eq:loss_supply}--%
        \ref{eq:loss_return}) requires only a linear loss term per
...
\Cref{fig:pareto} confirms this hierarchy: L3 achieves
...
\begin{figure}[ht]
  ...
  \caption{Accuracy--solve-time Pareto front. L3 represents the
  practical planning optimum: sub-percent cost deviation at
  tractable solve time. The dashed vertical line marks a 1\,h
  planning-tool time budget.}
  \label{fig:pareto}
\end{figure}
...
\label{tab:criteria}
\footnotesize
\begin{tabular}{@{} l p{5.5cm} p{3.2cm} l @{}}
\toprule
...
accounting underestimates emissions by ${\approx}9\,\%$ (772
...
resolution appears warranted whenever pipe length exceeds
...
post-upgrade HP/TES dispatch is verified through necessary
...
\Cref{app:hi_definition}) is used as a parametric study variable but has
...
windows combines linearisation error and transport-delay
...
The MIQCP reference is solved on two winter windows (January and
February 2025); a spring window (May) proved infeasible due to
...
This paper presents a controlled comparison that systematically
...
dispatch optimization. This is the first study
in which these three dimensions are varied one at a time within a
...
The research identified several continues research topics: storage-dispatch sensitivity on networks with longer transport delays; ring and meshed topologies; variable transport delay; supply-temperature optimization; joint dispatch-sizing comparison; full MINLP with native exponentials; stochastic variants; sub-hourly resolution; and 5th-generation bidirectional networks. Independent separation of the linearisation error from transport-delay activation via intermediate MIQCP runs is a
priority follow-up.
...
number~\textbf{03EN6057B} (Verbundvorhaben eProNet, Teilvorhaben:
Intelligenter W\"armenetzbetrieb; project period 2026--2028).
Project management was carried out by Forschungszentrum J\"ulich
GmbH (PT-J.ESN6). The authors thank Stefan Roth and e-con AG,
...
Zenodo at \cite{ruess_2026_calion} under MIT
license. Input time series are subject to NDA; anonymised summary
...
\noindent
\begin{tabular}{@{} >{\raggedright}p{1.5cm} p{5.0cm} @{}}
...
\noindent
\begin{tabular}{@{} >{\raggedright}p{1.5cm} >{\raggedright}p{1.8cm} p{3.2cm} @{}}
...
\noindent
\begin{tabular}{@{} >{\raggedright}p{1.8cm} >{\raggedright}p{1.2cm} p{3.5cm} @{}}
...
The heat pump COP is pre-computed from an analytical Lorenz-cycle
model:
$\mathrm{COP}_{h,t}^{\mathrm{WHR}} = \eta_{\mathrm{Lorenz}}
\cdot T_{\mathrm{sink,out}} / (T_{\mathrm{sink,out}} -
...
$\varepsilon_{\Delta p}^{\max} = R \bar{Q}^2 / (4K^2)$.
With $K{=}3$: 2.8\,\% of peak pressure drop.

\medskip\noindent
\captionof{table}{PWL error vs.\ $K$ (primary case).}
\label{tab:pwl_sensitivity}
\centering
\footnotesize
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} c c r l @{}}
\toprule
$K$ & $\varepsilon^{\max}$ & Vars & $\Delta Z$ \\
\midrule
3 & 2.8\% & 614k & $+0.35\%$ \\
5 & 1.0\% & 858k & -- \\
8 & 0.4\% & 1226k & -- \\
...
\midrule
\multicolumn{3}{@{}l}{Group mean, val.\ (6~nodes)}
  & 1.32 & & \\
\multicolumn{3}{@{}l}{Group mean, cal.\ (4~nodes)}
  & 2.32 & & \\
\multicolumn{3}{@{}l}{Group mean, excl.\ (4~nodes)}
  & 1.57 & & \\
\multicolumn{3}{@{}l}{All 14~nodes}
  & 1.68 & & \\
\bottomrule
\end{tabular*}
\begin{tablenotes}\scriptsize
...
across the full 2125\,m trunk corridor via a global multiplier
(${\times}1.330$ on nominal EN~253 $U$-values for all 8~trunk
pipes). The multiplier was determined by golden-section search
minimising MAE on the Oct--Feb winter subset. This approach
differs from the branch-sequential optimization-model calibration
(which concentrates losses in the terminal pipe at
$4.7{\times}$~nominal); both methods are valid for their
respective purposes (BCM: forward physics; L3/$\Lnl$: dispatch
optimization). RMSE: \SI{2.15}{\celsius}; bias:
...
$\leq 0.5\,\%$ gap (1.99\,\% at 4\,h limit for 744\,h window).
