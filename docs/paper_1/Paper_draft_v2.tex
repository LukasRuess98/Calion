...
\begin{highlights}
  ...
  \item Physics inclusion (losses) dominates spatial refinement by
        factor~\placeholder{X} in cost impact
  \item Piecewise-linear MILP deviates $<$\placeholder{X}\,\% from
        quadratic reference---adequate for planning
  \item Storage charge timing shifts up to \placeholder{X}\,h between
        copperplate and full network model
...
systematically varies both topology and physics fidelity as
independent experimental variables across five model variants within
a unified framework:
L1 (copperplate, no network physics),
L2 (simplified 7-zone multi-node with steady-state thermal losses),
L3 (detailed 15-node MILP with identical steady-state losses),
$\Lplus$ (detailed 15-node MILP with linearized pressure drop,
temperature-dependent loss propagation, and transport delay), and
$\Lnl$ (detailed 15-node MIQCP retaining native quadratic and
bilinear constraints as the highest-fidelity tractable reference).
This design yields three orthogonal comparisons:
topology effect (L1$\to$L2$\to$L3),
physics fidelity effect (L3$\to\Lplus$), and
linearization error ($\Lplus\to\Lnl$).
The framework is validated against measured operational data from a
...
and \ce{CO2} differences of \placeholder{Z}\,t/year; the
L3$\to\Lplus$ transition (extended physics) adds
\placeholder{X--Y}\,\% through pressure-drop-driven pumping costs
and transport-delay-induced dispatch shifts; while the
$\Lplus\to\Lnl$ comparison (linearization error) reveals that
piecewise linearization introduces a cost deviation of only
\placeholder{X.X}\,\%, validating the MILP approximation for
planning purposes. Thermal storage dispatch emerges as the primary
diagnostic variable: L1 shifts optimal charge timing by
\placeholder{X}\,hours, while $\Lplus$ shifts discharge timing by
\placeholder{X.X}\,hours relative to L3.

These findings yield a decision framework: L1 suffices for
screening; L2 is the minimum for storage-containing systems; L3
balances fidelity and tractability; $\Lplus$ is warranted when
pumping costs exceed \placeholder{X}\,\% of thermal losses or
transport times exceed \placeholder{X}\,h; $\Lnl$ serves as a
linearization validation benchmark.
\end{abstract}
...
\url{https://doi.org/10.5281/zenodo.XXXXXXX} under an MIT license.
...
\begin{credit}
\textbf{Lukas [Nachname]:} Conceptualization, Methodology, Software,
Validation, Formal analysis, Investigation, Data curation,
Writing~-- original draft, Visualization.
\textbf{[Betreuer]:} Supervision, Writing~-- review \& editing,
Funding acquisition.
\textbf{[Industriepartner]:} Resources, Data curation,
...
\textbf{Hypothesis:} Storage dispatch is the most sensitive indicator
of both topology adequacy and physics fidelity because temporal
arbitrage depends on holding costs (invisible to L1) and delivery
timing (invisible to L3). This is tested in
...
\begin{figure}[htbp]
  \centering
  \figref{F1}\quad\placeholder{Diagram showing the five model variants
  on two axes: topology detail ($x$-axis) and physics fidelity
  ($y$-axis). Arrows indicate the three controlled comparisons.}
...
0.4\,\% of peak pressure drop for $K=8$ (derivation in
\Cref{app:pwl}).

\paragraph{Choice of $K=8$}
The number of PWL segments balances approximation accuracy against
variable count. With $K=8$, the worst-case pressure-drop error is
0.4\,\% of peak. A sensitivity analysis on $K \in \{3, 5, 8\}$
(\Cref{app:pwl}) confirms diminishing returns: $K=5$ achieves 1\,\%
accuracy, while $K=8$ provides additional margin at acceptable
computational cost.
...
function of~$Q_{p,t}$ (cubic relationship) with $K=8$ breakpoints:
\begin{equation}
...
\midrule
Pressure drop   & PWL ($K{=}8$)       & Quadratic $RQ^2$ \\
...
gap ${\leq}\,0.01\,\%$ for MILP and ${\leq}\,0.1\,\%$ for MIQCP
(\texttt{NonConvex=2}). Identical random seeds. No warm-starting is
applied across levels; each instance is solved from scratch to prevent
solution bias.
...
\begin{figure}[htbp]
  \centering
  \figref{F2}\quad\placeholder{Schematic of the 15-node network showing
  trunk (j$_1$--j$_3$), South arm (j$_4$--j$_8$), East arm
  (j$_9$--j$_{15}$). HP/EBoiler marked at j$_{12}$.}
...
Pressure drop           & --  & --   & --   & PWL ($K{=}8$) & Quadratic \\
...
\midrule
$T_{\mathrm{sup}}$ at source (MAE)
  & \placeholder{X.X}\,\si{\celsius}
  & ${<}\,\SI{0.5}{\celsius}$
...
\begin{figure}[htbp]
  \centering
  \figref{FV1}\quad\placeholder{Time-series: measured vs.\ simulated
  $T_{\mathrm{sup}}$ at source for representative winter week.}
...
All results reported as relative differences.}
\label{tab:cost_co2}
\begin{tabular}{@{} l c c c c @{}}
\toprule
Level & $\Delta$\,Cost vs.\ L3 & $\Delta$\,Fuel &
  $\Delta$\,Elec.\ & $\Delta$\,\ce{CO2} \\
\midrule
L1 & $-$\placeholder{X.X}\,\%
   & $-$\placeholder{X.X}\,\%
   & $-$\placeholder{X.X}\,\%
...
\begin{figure}[htbp]
  \centering
  \figref{F3}\quad\placeholder{Stacked-bar cost decomposition for
  L1, L2, L3.}
...
\midrule
L3     & (ref.) & 0 & (ref.) & \\
$\Lplus$ & $+$\placeholder{X.X}\,\%
         & \placeholder{X.X}\,\% of total
         & $+$\placeholder{X.X}\,\% & \\
$\Lnl$  & $+$\placeholder{X.X}\,\%
         & \placeholder{X.X}\,\% of total
         & $+$\placeholder{X.X}\,\% & \\
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Transport delay:} $k_p = 1$ on far end shifts
        discharge timing by
        ${\approx}\,$\placeholder{X.X}\,h. Marginal for primary
        case; significant in synthetic analysis.
\end{enumerate}

\begin{figure}[htbp]
  \centering
  \figref{F5}\quad\placeholder{Waterfall: L3 $\to$ pumping $\to$
  loss reduction $\to$ delay shift = $\Lplus$ $\to$ lin.\ error =
  $\Lnl$.}
...
\begin{table}[htbp]
\centering
\caption{Linearization error ($\Lnl$ is reference).}
\label{tab:linearization}
\begin{tabular}{@{} l c c c @{}}
\toprule
Metric & $\Lplus$ & $\Lnl$ & $\Delta$ \\
\midrule
Total cost          & -- & -- & \placeholder{$\pm$X.X}\,\% \\
Total \ce{CO2}      & -- & -- & \placeholder{$\pm$X.X}\,\% \\
Annual pump energy  & -- & -- & \placeholder{$\pm$X.X}\,\% \\
HP full-load hours  & -- & -- & \placeholder{$\pm$X.X}\,\% \\
Storage cycles      & -- & -- & \placeholder{$\pm$X.X}\,\% \\
\bottomrule
\end{tabular}
\end{table}

The linearization error is \placeholder{X.X}\,\% in total cost---below
both the topology gap and the physics gap, validating $\Lplus$ as a
practical substitute for $\Lnl$.

\begin{figure}[htbp]
  \centering
  \figref{F6}\quad\placeholder{Scatter: hourly
  $P^{\mathrm{pump}}$ in $\Lplus$ vs.\ $\Lnl$; $R^2$ annotated.}
  \caption{Pumping-power agreement between PWL and quadratic
  formulations.}
  \label{fig:pump_lin}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \figref{F7}\quad\placeholder{Mean winter-week SOC profiles, 5
  variants overlaid.}
...
Level & Variables & Binaries & Quad.\ constr.\
  & Solve & Gap \\
\midrule
L1      & \placeholder{XX,XXX}    & \placeholder{X,XXX}
        & 0 & \placeholder{XX}\,s    & ${<}\,0.01\,\%$ \\
L2      & \placeholder{XX,XXX}    & \placeholder{X,XXX}
        & 0 & \placeholder{XXX}\,s   & ${<}\,0.01\,\%$ \\
L3      & \placeholder{XXX,XXX}   & \placeholder{XX,XXX}
        & 0 & \placeholder{X,XXX}\,s & ${<}\,0.01\,\%$ \\
$\Lplus$ & \placeholder{X,XXX,XXX} & \placeholder{XX,XXX}
        & 0 & \placeholder{XX,XXX}\,s & ${<}\,0.01\,\%$ \\
$\Lnl$  & \placeholder{XXX,XXX}   & \placeholder{XX,XXX}
        & \placeholder{XXX,XXX}
        & \placeholder{XXX,XXX}\,s & ${<}\,0.1\,\%$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure*}[!htbp]
  \centering
  \figref{F10}\quad\placeholder{Two-panel: (a)~L1$\to$L2 gap vs.\
  pipe length, color by HI. (b)~L2$\to$L3 gap vs.\ HI, color by
  storage ratio.}
...
\begin{figure}[htbp]
  \centering
  \figref{F11}\quad\placeholder{L3$\to\Lplus$ gap vs.\ cumulative
  delay $\tau_{\max}$, color by pipe length.}
  \caption{Physics-fidelity gap as a function of transport delay.}
  \label{fig:synth_l3lp_gap}
\end{figure}

The $\Lplus \to \Lnl$ gap remains below \placeholder{X.X}\,\% across
all 36~configurations with no systematic parameter dependence.

\begin{table}[htbp]
\centering
\caption{Effect magnitude hierarchy (10th--90th percentile).}
\label{tab:effect_hierarchy}
\begin{tabular}{@{} l l l @{}}
\toprule
Comparison & Gap range & Primary driver \\
\midrule
L1\,$\to$\,L2        & \placeholder{X.X--X.X}\,\% & Pipe length \\
L2\,$\to$\,L3        & \placeholder{X.X--X.X}\,\% & Demand HI \\
L3\,$\to$\,$\Lplus$  & \placeholder{X.X--X.X}\,\% & Length, $\tau$ \\
$\Lplus$\,$\to$\,$\Lnl$ & \placeholder{X.X--X.X}\,\% & None \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Gap stability across scenarios.}
...
All gap signs are stable. Pump efficiency ($\pm 20\,\%$) shifts the
L3\,$\to$\,$\Lplus$ gap by
$\pm$\placeholder{X.X}\,pp but leaves other gaps unchanged.
...
\begin{enumerate}
  ...
  \item \textbf{Extended physics matter for long networks.}
        ...
        \placeholder{X}\,\% of thermal losses.
  \item \textbf{Linearization is adequate for planning.}
        PWL with $K=8$ introduces negligible error. MILP solvers
        suffice.
\end{enumerate}
...
\textbf{$\Lplus$}
  & Pipe ${>}\,\SI{5}{\kilo\metre}$ OR $\tau_{\max} > \SI{1}{\hour}$
    OR pumping ${>}\,$\placeholder{X}\,\% of thermal loss
...
Across all comparisons, storage dispatch metrics prove the most
sensitive indicators:
%
\begin{itemize}[nosep]
  \item L1\,$\to$\,L2: charge timing shifts \placeholder{X}\,h.
  \item L3\,$\to$\,$\Lplus$: discharge shifts \placeholder{X.X}\,h.
  \item $\Lplus$\,$\to$\,$\Lnl$:
        ${<}\,$\placeholder{X.X}\,h (negligible).
\end{itemize}

\textbf{Recommendation:} If charge/discharge timing differs by
${>}\,\SI{1}{\hour}$ between levels, the lower-fidelity model is
inadequate.
...
\placeholder{X--Y}\,\% of total network emissions.
...
\begin{enumerate}[nosep]
  \item \textbf{Topology effect (RQ1):} L1\,$\to$\,L2 dominates:
        cost underestimation of \placeholder{X}\,\%, \ce{CO2}
        underestimation of \placeholder{X}\,\%.
  \item \textbf{Physics fidelity (RQ2):} L3\,$\to$\,$\Lplus$ adds
        \placeholder{X--Y}\,\% through pumping and delay effects.
  \item \textbf{Linearization error (RQ3):}
        $\Lplus$\,$\to$\,$\Lnl$ gap
        ${<}\,$\placeholder{X}\,\%, validating MILP approximations.
  \item \textbf{Generalizability (RQ4):} L1\,$\to$\,L2 scales with
        pipe length; L2\,$\to$\,L3 with HI; L3\,$\to$\,$\Lplus$ with
        length and~$\tau$.
  \item \textbf{Diagnostic:} Storage charge/discharge timing shifted
        by up to \placeholder{X}\,h between L1 and $\Lnl$.
\end{enumerate}
...
\midrule
3 & 2.8 & ${\approx}\,370$k & \placeholder{X.X}\,\% \\
5 & 1.0 & ${\approx}\,615$k & \placeholder{X.X}\,\% \\
8 & 0.4 & ${\approx}\,1{,}226$k & \placeholder{X.X}\,\% \\
\bottomrule
\end{tabular}
\end{table}
...
< \placeholder{X.XX}$\,K, i.e.,
${<}\,$\placeholder{X}\,\% relative to the 30--60\,K driving force.
...
Darcy--Weisbach at optimized flows: \placeholder{XX}\,kW peak,
${<}\,$\placeholder{X}\,\% of thermal losses.
