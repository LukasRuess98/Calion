%% ============================================================
%% NOMENCLATURE -- v3, Applied Energy house style.
%% Front matter (immediately after \maketitle). Plain two-column symbol->definition
%% list: no table rules, no repeated headers, units folded into the definition in [..];
%% dimensionless quantities carry no unit. The fidelity-level block keeps its T x P code
%% column as a small ruled table, since it is a genuine lookup table, not a symbol list.
%%
%% CHANGES v2 -> v3:
%%  (a) level labels set upright (\textup), not in math mode
%%  (b) Status column added: optimised vs forward-evaluated (taxonomy consistency)
%%  (c) ZN given an explicit purpose (Section 3.4)
%%  (d) NL now names both roles: forward reference AND weekly re-solve
%%  (e) C^pump REMOVED -- pump electricity enters the electricity bus and is paid
%%      through grid procurement; a standalone term would double-count
%%  (f) b (loss burden) and HI added
%%  (g) alpha_s wording fixed to match the (1-alpha_s)^{Delta t} form of Eq. (11)
%%  (h) \arraybackslash added after \raggedright in p-columns
%% ============================================================

{\footnotesize

%%--- Fidelity levels and evaluation quantities (small lookup table, full width) ---
...
\ZN      & T1P1 & optimised   & Zone-aggregated $+$ loss (aggregation sensitivity, Section~\ref{sec:zone}) \\
...
\NLref   &      & reference   & Nonlinear forward evaluator and bounded weekly re-solve; not a ladder rung \\
\midrule
...
%%--- Symbol->definition lists, two columns, no rules ---
...
\nom{\mathcal{G}_n,\mathcal{S}_n}{Generators/storage at node~$n$}
...
% CHANGED: alpha_s is the fractional standing loss per hour entering as
% (1-alpha_s)^{Delta t} in Eq. (11); it is not a rate with unit h^-1.
...
% CHANGED: HI added (used in Table 6, Table 20, Appendix E)
\nom{\mathrm{HI}}{Demand heterogeneity index (Appendix~\ref{app:hi})}
...
% REMOVED: C^{pump}. Pump electricity enters the electricity balance, Eq. (4),
% and is paid through C^{en}; a standalone term would double-count it.
...
\nom{E_t^{\mathrm{CO_2,grid}}}{Grid CO$_2$ emissions [t]}
\nom{E_{g,t}^{\mathrm{CO_2,fuel}}}{Fuel CO$_2$ emissions [t]}
\nom{W_{p,t}}{Bilinear auxiliary, $=Q_{p,t}^2$ [MW$^2$]}
\nom{u_{h,t}}{Heat-pump on/off, binary}
\nom{z_t}{Grid import mode, binary}
\nom{w_{p,t,k}}{SOS2 weights, $[0,1]$}
