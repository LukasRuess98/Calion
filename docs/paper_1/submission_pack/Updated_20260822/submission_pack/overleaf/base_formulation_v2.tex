...
% CHANGED: C^pump removed from the objective. Pump electrical power enters the
% electricity balance, Eq. (4), and is therefore paid through C^en; listing it
% separately would double-count it.
\paragraph{Objective function.}
The optimiser minimises total annual operating cost: electricity procurement net of
feed-in revenue, fuel, heat-curtailment penalties, carbon charges and a demand charge on
peak grid import,
%
\begin{equation}
...
zero at every level below $\Lthree$ and is computed from Eq.~\eqref{eq:pumppower} from
...
Two terms of $Z$ are accounting conventions rather than operator cash flows, and the
economic cost is reported separately from the solver objective throughout; the
...
% CHANGED: removed the visible reference to a reviewer request. The design
% rationale stands on its own and must not name the review process.
...
% CHANGED: the match to measurement is quantified (about one percent, Section 3.1)
% rather than asserted, and "true network loss" replaced -- the measurement is the
% reference, not a known truth.
...
trunk-only loss to match the measured total by inflating terminal-pipe coefficients cannot be
justified as insulation ageing. Second, what such a multiplier would absorb is the
...
level $\Lfour$. The node-resolved levels consequently \emph{undercount} the measured network
loss by construction, and the modelled total matches the measurement to about one percent
only once the laterals are added (Section~\ref{subsec:validation}). This is a result rather
...
\end{equation}
%
% CHANGED: alpha_s is a dimensionless fractional standing loss per hour in the
% (1-alpha_s)^{Delta t} form, not a rate with unit h^-1.
