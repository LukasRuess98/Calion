...
## NEW: `sections/` folder (required for the build)

The manuscript now `\input`s five merged section files. Create `01_latex_build/sections/`
and copy them in from `03_draft_revision/02_revision_hand_in/`:

| File | Replaces |
|---|---|
| `introduction_opening_v2.tex` | `<<KEEP:intro-motivation>>` |
| `related_work_v2.tex` | the three `<<KEEP:rw-*>>` markers |
| `base_formulation_v2.tex` | the six `<<KEEP:objective/balances/losses/hp/storage/emissions>>` |
| `validation_results_v2.tex` | `<<KEEP:validation-results>>` |
| `limitations_v2.tex` | `<<KEEP:limitations-other>>` |

**Not yet wired:** `extended_physics_validation_implementation_v2.tex` bundles three
subsections (§2.6 extended physics, §2.8 validation protocol, §2.9 computational setup)
whose targets already contain drafted prose. It needs splitting into three files and a
careful merge against that prose rather than a blind `\input`, or content will duplicate.

## Structure

Restructured to the four sections of the Applied Energy exemplar (Akter et al. 2024):
Introduction (literature as §1.1) → Experimental design and methodology (case study folded
in) → Results and discussion (combined) → Conclusions. See `STRUCTURE_v2.md`.
