%% ============================================================
%% BASE FORMULATION -- v2 merge, drop-in replacement for the
%% <<KEEP:objective>> <<KEEP:balances>> <<KEEP:losses>> <<KEEP:hp>>
%% <<KEEP:storage>> <<KEEP:emissions>>  markers under \subsection{Base formulation}.
%%
%% Source: v1 lines 724-962, remapped to the v2 taxonomy.
%%   v1 L1  -> \CP      (copperplate)
%%   v1 L2  -> \ZN      (zone-aggregated)
%%   v1 L3  -> \Lone    (node-resolved + trunk loss, the BASELINE)
%%   v1 L3+ -> \Ltwo/\Lthree ;  v1 L3NL -> \Lsix/\NLref
%% Every "L1/L2/L3" in the v1 text therefore had to be re-pointed, not copied.
%%
%% TWO SUBSTANTIVE CHANGES beyond remapping, both required by the revision:
%%  (a) v1 closed the energy-balance paragraph by saying the missing pipe-loss and
%%      spatial-flow terms "are the mechanism driving the L1->L2 cost gap". That is the
%%      confound R2.2 objected to -- the two terms are named together and priced together.
%%      Rewritten to show the two controls are obtained by switching the terms
%%      INDEPENDENTLY, which is what makes the decomposition exact.
%%  (b) The loss paragraph now states the defensible-U calibration and the last-mile
%%      placement rule (laterals enter only at L4), replacing v1's silent multiplier.
%%
%% PACKAGE NOTE: v1 used \ce{CO2} (mhchem) and \si{\celsius}/\SI (siunitx).
%% paper_COMPILE.tex writes CO$_2$ and °C directly, so those macros are NOT used here.
%% If mhchem/siunitx are added to the preamble later, they can be restored.
%% ============================================================
...
\end{equation}
%
where $C^{\mathrm{pump}} = 0$ at every level below $\Lthree$ and enters from $\Lthree$
upward (Section~\ref{sec:extended}). With $\Delta t = 1$\,h and $t$ indexing all
...
$f^{\mathrm{yr}}$ an annualisation factor; and $P^{\mathrm{peak}}$ [MW] the annual peak
grid import entering the demand charge $\lambda^{\mathrm{dem}}$.

Two terms of $Z$ are accounting conventions rather than operator cash flows, and we
report the economic cost separately from the solver objective throughout; the
...
generators and storage, with no spatial routing:
%
\begin{equation}
...
control condition Reviewer~2 asked for, and it is the reason the decomposition in
...
constrained to a defensible range for the pipe class rather than fitted freely; the
original submission raised terminal-pipe coefficients by a large multiplier to force the
trunk-only loss to match the measured total, which cannot be justified as insulation
ageing. Second, what that multiplier was absorbing is the \emph{last mile}, the service
laterals and substations a trunk-only model does not represent, and it is therefore
placed where it physically belongs, entering only at the station-resolved level $\Lfour$.
The node-resolved levels consequently \emph{undercount} the true network loss by
construction, and the modelled total matches the measurement only once the laterals are
added. We treat this as a result rather than a defect: it is a quantitative statement of
what a trunk-resolved model cannot see.
...
convenience, and we return to it in the limitations: a model with endogenous COP would
...
\end{equation}
%
The coefficient $\alpha_s$ [h$^{-1}$] represents standing losses of the vessel, and the
exponential form makes the discretisation independent of the time step. A cyclic
condition $E_{s,|\mathcal{T}|} = E_{s,0}$ enforces energy-neutral operation over the year,
preventing artificial depletion or accumulation.
