...
carries the same sign. That is a displaced model, not a wrong one, by an amount squarely
inside the $5$--$15$\,°C offset of a three-way mixing valve; the corridor comparison shows the
same signature ($-7.73$\,°C bias). Neither obstacle is removable by a better pipe model, and
...
The figures here differ from the original submission, and both are correct. The original
reported a $1.32$\,°C mean supply-temperature error over six of the fourteen nodes across a
744-hour winter window; the figure above is the far-end node over the full annual record,
unselected---the more conservative view, since the far end runs the mixing valve closest to
fully open and the annual record admits no favourable window. Appendix~\ref{app:per_node_mae}
