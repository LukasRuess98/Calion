...
to matter, though not necessarily monotonically; we therefore read the nulls as conservative
\emph{within the tested regime}, and a reader operating outside it should treat them as
indicative rather than as guarantees. The loss-dominance result is more robust: the
decomposition itself is an exact algebraic identity on the four optimised costs, and
empirically the loss term dominates across all 135 synthetic networks, spanning two orders of
magnitude in loss burden. Its magnitude remains conditional on the tested systems, but within
that regime it does not hinge on any single one of the four assumptions above. That is why it,
and not the nulls, is what the title claims.
