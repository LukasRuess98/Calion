...
600 dpi, PNG+PDF+PGF). Reproducible: re-run after any results change.
  ...
  F_decomp  loss/topology/interaction (Memmingen) + 42-net synthetic distribution
...
def f_decomp():
    ...
    ax2.set_title("(b) Synthetic factorial (42 networks)", fontsize=9)
    fig.tight_layout()
    _save(fig, "F_decomp")
...
def f_rule():
    ...
    ax.scatter(syn["lambda"], syn["b_meas_pct"], s=16, color=SILVER, edgecolor="k",
               lw=0.3, label="synthetic (42)", zorder=3)
    if len(mem):
