...
A residual concern is that the schedules are optimised on the solver objective, so an
objective aligned to the reported economic cost could in principle select different
dispatches and shift the decomposition. This is tested directly. The storage-cycling
penalty is the only objective term that both differs from the economic cost and is a
well-posed optimisation term (the carbon self-use credit is a post-hoc reporting
allocation whose inclusion in the objective would reward selling electricity purely to
shed carbon, and is therefore excluded by construction; the objective already minimises
gross carbon, for which the gross-basis figures above are the objective-consistent
report). Removing the cycling penalty and re-solving the four decomposition levels to the
same $0.01$\,\% gap lowers every level's economic cost by a near-uniform $3.5$\,\%, so the
loss main effect is essentially unchanged: the loss share moves from $95.8$\,\% to
$95.6$\,\% and the topology share from $4.7$\,\% to $5.0$\,\%. The decomposition is thus
stable to the objective--economic distinction, not merely arithmetically separated from
it (\texttt{loss\_share\_econobj\_sensitivity.csv}).
