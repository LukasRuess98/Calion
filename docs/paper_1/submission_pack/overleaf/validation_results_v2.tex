...
computed difference between generation and delivered demand. No independent measurement of
loss is claimed, as this would require metered supply-side energy that the site does not
record, but the agreement in delivered energy fixes the scale at which loss enters the cost.
The source return temperature matches to \textbf{2.08}\,°C against a \(2.5\)\,°C gate and,
...
\(0.01\,\%\). On the economic-cost basis, the relaxation ordering
\(\mathrm{cost}(\CP) \leq \mathrm{cost}(\Lone)\) also holds for every one of the 135 synthetic
instances. A violation of either condition would indicate a formulation error rather than a
finding.