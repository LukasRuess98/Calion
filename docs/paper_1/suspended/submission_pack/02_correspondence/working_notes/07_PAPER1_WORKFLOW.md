...
- **Energy-balance non-closure — VERIFIED 2026-08-10, the earlier explanation was WRONG.**
  On L1 (T2P1_defU) the gap is **~10 %** (supply 16.03 vs sink 17.78 GWh; not "18–30 %"),
  and it is **NOT** electric-boiler/P2H heat — EK is **zero** in this run, so it cannot be
  the cause. The residual (≈1.75 GWh) instead matches an **unattributed grid-electricity
  term**: P_buy (4.66 GWh) exceeds P_hp_el+P_ek_el (2.87) by ~1.8 GWh, i.e. purchased
  electricity whose heat conversion is not fully tallied in `dispatch_hourly.csv`. It IS an
  export/reporting artifact (economics.csv is unaffected — the optimised cost is right), but
  the fix is in the **grid-electricity→heat attribution**, not a P2H term. TODO: confirm the
  magnitude across all levels (only L1 checked) and locate the missing export column before
  writing any number in the manuscript.
