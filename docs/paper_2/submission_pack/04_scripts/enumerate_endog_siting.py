...
def _disable_pressure_slack() -> None:
    """Monkeypatch ThermalNodeBlock.attach to force pressure_slack OFF.

    2026-08-30: the campaign (2026-07-24) predates the `pressure_slack` feasibility
    term (added 2026-08-07). A current-code re-solve WITH it diverges from the frozen
    MM-S4 objective (493k vs 299k) because the 8760 extra slack vars make the model
    harder to converge. To re-solve MM-S4 consistently with the frozen dispatch runs
    we disable pressure_slack (matching campaign behaviour, which had no such term).
    NOTE: without slack, a j_13 data-artifact hour may render the full-year solve
    INFEASIBLE -- if so the re-solve fails loudly, which is the correct signal.
    """
    from calion.models.blocks.thermal_node import ThermalNodeBlock
    _orig_attach = ThermalNodeBlock.attach

    def _patched_attach(model, time_set, config, buses, network_pipes):
        config = {**config, "pressure_slack": False}
        return _orig_attach(model, time_set, config, buses, network_pipes)

    ThermalNodeBlock.attach = staticmethod(_patched_attach)


def _run_single_pair(scenario_id: str, hp_site: str, tes_site: str,
                      time_limit: int, outdir: Path, gurobi_threads: int = 4,
                      no_pressure_slack: bool = False) -> dict:
    ...
    if no_pressure_slack:
        _disable_pressure_slack()
        logger.info("[ENUM] pressure_slack DISABLED (campaign-consistent re-solve)")
...
def _orchestrate(scenario_id: str, time_limit: int, concurrency: int, outdir: Path,
                  skip_existing: bool = False, gurobi_threads: int = 4,
                  no_pressure_slack: bool = False) -> None:
...
def main() -> None:
    ...
    ap.add_argument("--no-pressure-slack", action="store_true",
                     help="Disable the 2026-08-07 pressure_slack term so the re-solve is "
                          "consistent with the pre-08-07 frozen campaign (see _disable_pressure_slack)")
    args = ap.parse_args()
    ...
    if args.summarize_only:
        ...
        _orchestrate(args.scenario, args.time_limit, args.concurrency, args.outdir,
                     ...
                     no_pressure_slack=args.no_pressure_slack)
    else:
