...
def compute_all_kpis(out_base: Path) -> Path:
    ...
    # GUARD (2026-08-30): only the canonical full-year baseline may serve as the
    # cost/CO2 reference. Diagnostic baselines (e.g. BC-MM-DIAGJAN, a January-only
    # short run) also carry baseline=True and network=memmingen; because scen_dirs
    # is sorted alphabetically, "BC-MM-DIAGJAN" sorted AFTER "BC-MM" and silently
    # overwrote the real baseline -> every Memmingen cost_reduction_pct was computed
    # against a ~7x-too-small January denominator (BC-MM -604% vs itself; MM-S5-HK2
    # -289% despite LCOH being LOWER than baseline). Skip any diagnostic/throwaway id.
    _DIAG_MARKERS = ("DIAG", "TEST", "ZZ", "NEVER", "DIAGSHORT", "DIAGJAN", "TVLFIX")

    def _is_diagnostic(sid: str) -> bool:
        return any(m in sid.upper() for m in _DIAG_MARKERS)
    ...
    for scen_dir in scen_dirs:
        ...
        if meta.get("baseline"):
            sid = meta.get("scenario_id", scen_dir.name)
            if _is_diagnostic(sid):
                logger.info("Skipping diagnostic baseline %s (not a cost reference)", sid)
                continue
