...
def build_f4() -> None:
    print("F4 - heat-curve coupling of TES volume, COP and TAC (2x3):")
    df = _kpis()
    if df is None:
        return
    df = _annotate_net(df)
    _style.apply_rcparams()
    nets = ["MM", "SB"]                          # Memmingen top row, Stadtbach bottom
    fig, axes = plt.subplots(len(nets), 3, figsize=(_style.COL_DOUBLE_IN, 4.7))
    any_data = False
    for i, net in enumerate(nets):
        ...
        # locate the optimal (min-TAC) stage for the star marker
        tac = pd.to_numeric(sub["TAC_eur_per_a"], errors="coerce").to_numpy(float)
        opt = int(np.nanargmin(tac)) if not np.all(np.isnan(tac)) else None
        for j, (col, ylab, scale) in enumerate(_F4_COLS):
            ax = axes[i][j]
            y = pd.to_numeric(sub[col], errors="coerce").to_numpy(float) * scale
            ax.plot(x, y, "-o", ms=8, lw=2, color=color, zorder=3)
            any_data |= not np.all(np.isnan(y))
            # star the cost optimum on the TAC panel + direct-label it
            if col == "TAC_eur_per_a" and opt is not None and not np.isnan(y[opt]):
                ax.scatter([x[opt]], [y[opt]], marker="*", s=210, color=color,
                           edgecolor="white", linewidth=0.8, zorder=5)
                ax.annotate(f"{y[opt]:,.0f}", (x[opt], y[opt]),
                            textcoords="offset points", xytext=(0, 10),
                            ha="center", fontsize=7.5, color=_style.INK)
            ax.set_xticks(x)
            ax.set_xticklabels(_STAGES if i == len(nets) - 1 else [])
            ax.set_ylabel(ylab, fontsize=8)
            ax.grid(axis="y")
            ax.margins(y=0.22)
            if i == 0:
                ax.set_title(["Storage volume", "Coefficient of performance",
                              "Total annual cost"][j], fontsize=9)
        # row (network) identity label at the far left
        axes[i][0].annotate(NET_LABEL[net], xy=(-0.42, 0.5),
                            xycoords="axes fraction", rotation=90, va="center",
                            ha="center", fontsize=9.5, fontweight="bold", color=color)
    axes[-1][1].set_xlabel("Heat-curve stage  (HK0 → HK2 = lower supply temperature)",
                           fontsize=8.5)
    fig.suptitle("Heat-curve stage jointly sets storage volume, COP and cost "
                 "(fixed siting; star = cost optimum)", fontsize=10, y=1.01)
    fig.subplots_adjust(hspace=0.18, wspace=0.32, left=0.11)
    if not any_data:
        print("  [PARTIAL] F4: reference families lack TES/COP/TAC data")
...
def build_f5() -> None:
    ...
    # Cost components use a categorical triad DISTINCT from the network hues
    # (blue=Memmingen, green=Stadtbach are reserved for entity identity across the
    # figure set — dataviz: one colour, one meaning). Teal/lime/orange here.
    cats = [("CAPEX (annualised)", _style.FHG_TEAL),
            ("Energy OPEX", _style.FHG_LIME),
