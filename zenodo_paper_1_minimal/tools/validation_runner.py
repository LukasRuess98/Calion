...
# Resolve output dirs: results/ in standalone package, output/ in dev repo
_RUNS_BASE     = ROOT / "results" if (ROOT / "results").exists() else ROOT / "output" / "paper_runs"
LEGACY_DIR     = _RUNS_BASE / "legacy"
L3_DIR         = _RUNS_BASE / "L3"
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Graceful skip when the NDA operational data is not present
    if not data_path.exists():
        print("\n[VALIDATION SKIPPED]")
        print(f"  Measured operational data not found: {data_path}")
        print("  The primary-case validation requires the raw operational data")
        print("  from the Memmingen network, which cannot be shared publicly")
        print("  due to a non-disclosure agreement.")
        print("  Pre-computed validation KPIs are available in results/validation_kpis.json")
        return 0
