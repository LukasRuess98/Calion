...
**1 · Figures.** Build F2b, F11, F13, F14, F16, F19 from `FIGURES.md`, in that order. F2b is ten
minutes and is a go/no-go check on the paper's structure. F11 is the graphical abstract. F16
(dual of the connection constraint = EUR/MW value of capacity) is nearly free and is probably the
most useful figure for a practitioner readership.

**2 · Intensive network use.** § 19 Abs. 2 S. 2 StromNEV grants an individual network charge at
≥ 7,000 utilisation hours and ≥ 10 GWh/a. Electrification pushes sites *towards* that threshold —
an economic driver working against peak shaving that is currently missing. Implement as a
post-hoc eligibility check plus an optional two-pass re-solve with the discount applied; do not
add a binary to the LP (rule 6).
