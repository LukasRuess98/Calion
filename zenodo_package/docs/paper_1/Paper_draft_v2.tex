...
\begin{highlights}
  ...
  \item Storage charge/discharge timing shifts ${<}\,0.2\,h$ between
        copperplate and full network model (primary case)
...
(physics inclusion) drives cost differences of 12\,\%
and \ce{CO2} differences of 63\,t/year; the full
L1$\to$L3 hierarchy yields 74\,t/year \ce{CO2} underestimation; the
L3$\to\Lplus$ transition (extended physics) adds
$0.11$\,\% through pressure-drop-driven pumping costs
...
$0.35$\,\%, validating the MILP approximation for
planning purposes. Thermal storage dispatch emerges as the primary
diagnostic variable: charge/discharge timing differences remain
${<}\,0.2\,h$ across all levels in the primary case, confirming that
dispatch structure rather than timing dominates model sensitivity.
...
pumping costs exceed $1\,\%$ of thermal losses or
transport times exceed $1\,h$; $\Lnl$ serves as a
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{../../results/figures/F1_experimental_design.pdf}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{../../results/figures/F2_network_topology.pdf}
...
$^{\dagger}$Flow MAPE reflects volumetric measurement uncertainty inherent in the
pre-upgrade network (pressure-differential metering); model mass-flow balance is
verified via energy balance closure (0.70\,\%).
\end{table}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{../../results/figures/FV1_validation_timeseries.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{../../results/figures/F3_cost_topology.pdf}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{../../results/figures/F5_cost_waterfall.pdf}
...
The linearization error is $+0.35$\,\% in total cost (January 2025 window),
below both the topology gap ($13$\,\%) and the physics gap ($0.11$\,\%
on an annual basis). The $\Lnl$ reference reached a 1.99\,\% MIP gap at the
4-hour time limit; the true MIQCP optimal may reduce the linearization error
further, but the $\Lplus$ approximation is conservative. This validates
$\Lplus$ as a practical substitute for $\Lnl$.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{../../results/figures/F7_TES_SOC_comparison.pdf}
...
$^{\dagger}$Bilinear constraints from linearized temperature--flow products; from
\texttt{level\_consistency.json} note. Variable counts for L2--$\Lplus$ not
extracted at solve time (logged separately).
\end{table}
...
\begin{figure*}[!htbp]
  \centering
  \includegraphics[width=\textwidth]{../../results/figures/F10_node_topology_heatmap.pdf}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{../../results/figures/F11_critical_path_profile.pdf}
...
\vspace{2pt}
\raggedright\footnotesize
$^{*}$L1 is a copperplate single-bus model; demand-fraction scaling of
individual consumer nodes does not alter the aggregated energy balance.
The cold/warm L1--L3 gap therefore cannot be isolated and is excluded.
\end{table}
...
%
\begin{itemize}[nosep]
  \item L1\,$\to$\,L2: charge timing shifts ${<}\,0.2\,h$ (primary case).
  \item L3\,$\to$\,$\Lplus$: discharge shifts ${<}\,0.1\,h$.
  \item $\Lplus$\,$\to$\,$\Lnl$:
        ${<}\,0.1\,h$ (negligible; January window).
\end{itemize}
